...
class FundusSegmentation(Dataset):
    ...
    def __init__(self,
        ...
        imagelist = glob(self._image_dir + "/*.png") + \
            glob(self._image_dir + "/*.jpg")
        for image_path in imagelist:
            gt_path = image_path.replace(
                'image', 'mask').replace('jpg', 'bmp')  # Refugue
            # gt_path = image_path.replace('image', 'mask').replace('jpg', 'png') # Rimone-v3
            self.image_list.append(
                {'image': image_path, 'label': gt_path, 'id': testid})
    ...
    def _read_img_into_memory(self):
        ...
        for index in range(img_num):
            self.image_pool.append(Image.open(
                self.image_list[index]['image']).convert('RGB'))
